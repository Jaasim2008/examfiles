...
$filteredData = array_filter($data, function ($item) use ($selectedYear, $selectedSub) {
    return (!$selectedYear || $item['Year'] == $selectedYear) &&
           (!$selectedSub || $item['Class/Subject'] == $selectedSub);
});
...
<body class="bg-base flex flex-col min-h-screen">
    <section class="hidden md:inline fixed grid grid-rows-3 right-0 mr-6 bg-secondary gap-1 border border-t-0 divide-y divide-primary rounded-b-md" style="border-color: var(--primary);">
        <button class="p-1 hover:text-secondary flex gap-1 items-center" onclick="toggleTheme()"><img class="w-[28px] invert" src="theme.svg"></img> Switch Theme</button>
        <a href="aboutme.php" class="p-1 hover:text-secondary flex gap-1 items-center"><img class="w-[28px] invert" src="person.svg"></img> About Me</a>
        <a href="https://buymeacoffee.com/jaazim" target="_blank" class="p-1 hover:text-secondary flex gap-1 items-center"><img class="w-[28px] invert" src="heart.svg"></img> Support Me</a>
    </section>
        ...
        <div class="overflow-x-auto">
            ...
            <tbody>
                ...
                <?php foreach ($filteredData as $row): ?>
                    ...
                    <td class="border border-gray-700 px-4 py-2"><?= htmlspecialchars($row['Year']) ?></td>
                    <td class="border border-gray-700 px-4 py-2"><?= htmlspecialchars($row['Class/Subject']) ?></td>
